...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        mainPanel(
            ...
            # fluidRow(
            #     splitLayout(cellWidths = c("33%", "33%", "33%"), plotOutput("kendallPlot_10", height = 800), plotOutput("kendallPlot_11", height = 800), plotOutput("kendallPlot_12", height = 800))
            # )
            plotOutput("kendallPlot_11", height = 800, width = 600),
            # plotOutput("kendallPlot_12", height = 800, width = 600)
        )
    )
)
...
server <- function(input, output) {
    ...
    output$kendallPlot_10 <- renderPlot({
        
        date_of_study = "2020-10-01"
        ...
        if(nrow(kendall.cor.new) > 0) {
            ...
            kendall.cor.new %>% 
                ...
                labs(
                    y = "Correlation",
                    x = NULL,
                    title = paste("Factors Associated with COVID-19 Deaths by Oct for", names(which(state.list == input$state_choice))),
                    fill = "Relationship",
                    color = "Relationship"
                ) +
                theme_minimal() +
        ...
        # ggsave(paste(input$state_choice, "_kendall_.png", sep=""), width = 7, height = 9)
    }, bg = "transparent")
    
    output$kendallPlot_11 <- renderPlot({
        
        date_of_study = "2020-11-18"
        ...
        if(nrow(kendall.cor.new) > 0) {
            ...
            kendall.cor.new %>% 
                ...
                labs(
                    y = "Correlation",
                    x = NULL,
                    title = paste("Factors Associated with COVID-19 Deaths by Nov for", names(which(state.list == input$state_choice))),
                    fill = "Relationship",
                    color = "Relationship"
                ) +
                theme_minimal() +
    ...
    output$kendallPlot_12 <- renderPlot({
        
        date_of_study = "2020-12-01"
        ...
        if(nrow(kendall.cor.new) > 0) {
            ...
            kendall.cor.new %>% 
                ...
                labs(
                    y = "Correlation",
                    x = NULL,
                    title = paste("Factors Associated with COVID-19 Deaths by Dec for", names(which(state.list == input$state_choice))),
                    fill = "Relationship",
                    color = "Relationship"
                ) +
                theme_minimal() +
